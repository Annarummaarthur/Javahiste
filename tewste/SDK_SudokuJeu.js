...
| Ce script doit être initialisé par le module "SudokuInit.js"                 |
| variables passées: tableaux 9x9 Grille et Masque ( caractères '0' à '9' )    |
+------------------------------------------------------------------------------+
*/
//
...
function SDK_sudoku() {
    ...
    Objs.disabled= true;
    texteHTML='<fieldset>';
    texteHTML+='<legend>gomme et saisie des chiffres</legend>';
    for (i=0;i<10;i++){
...
function SDK_gestion(c){
    ...
    switch(c){
      ...
      case 2: // commande "pause/continue"
        // gestion des boutons "commandes" et du chronomètre
      ...
      case 3: // commande "rejouer"
        // arrêt et remise à zéro du chronomètre
        clearInterval(SDK_chrono);
        // réaffichage du jeu
        SDK_sudoku()
        // mode de gestion des clics/cases
...
function SDK_case(i,j){
    ...
    // mode 1 = affiche dans la case la sélection mémorisée dans SDK_memchif
    if (SDK_modecase==1){
    ...
    // status (cases non jouées ou fausses)
    Objs=getObjs('SDK_status');
    reste=0;
    for (i=0;i<9;i++){
      for (j=0;j<9;j++){
    ...
    if (reste!=0) {
      ...
      // on désactive la barre horizontale
      Objs=getObjs('SDK_menuH');
      Objs.disabled= true;
      for (i=0;i<10;i++){
        Objs=getObjs('SDK_bouton'+i);
      ...
      // pas de chiffre mémorisé ni affiché
      SDK_memchif='';
      Objs=getObjs('SDK_chx');
      Objs.innerHTML='&nbsp;';
...
//--- gestion de la sélection du chiffre
function SDK_mem(chiffre){
    var chiffre, v, i;
    chiffre= chiffre.toString();
    // grise le bouton sélectionné: '0' à '9' ('0'= gomme)
    v= parseInt(chiffre);
    for (i=0;i<10;i++){
      Objs=getObjs('SDK_bouton'+i);
	  (i==v)? Objs.disabled= true: Objs.disabled= false;
    }
    // mémorise et affiche la sélection
    Objs=getObjs('SDK_chx');
    switch (chiffre) {
      case '':
        Objs.innerHTML='&nbsp;';
