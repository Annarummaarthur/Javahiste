...
<head>
    ...
    <link rel="stylesheet" href="style/sudoku.css">
    <script>
...
<body>
    ...
    function generer_grille_sudoku() {
        ...
        for ($j=0; $j<9; $j++) {
         ...
         // Remplacer 5 cases aléatoires par des zéros pour représenter les cases vides
         for ($k=0; $k<20; $k++) {
            $i = rand(0, 8);
            $j = rand(0, 8);
            $tab[$j][$i] = 0;
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
        ...
        if ($sudokuCorrect) {
            header('Location: fins.php?result=success&start_time=' . $start_time);
            exit;
        } else {
            header('Location: fins.php?result=failure&start_time=' . $start_time);
            exit;
        }
    }
